...
## 六之二、门是不是真门（`gate_probe.py`）

上面那七条查的是门**在不在**（workflow 里有没有那一行、阈值有没有放宽），**查不出门是不是空的**。
空门有三类成因，都不会让任何测试变红，CI 还挂着绿勾：

1. **正则写错**。实测：`#\[error\(\s*(?:"[^"]*")?[^")]*[一-鿿]` 的可选组会贪婪吃掉整个字符串，
   之后再要求 `[^")]*` + 中文 ⇒ 永远匹配不上，`ident_gate` 的 I2 一度就是道空门。
2. **路径过滤没匹配到任何文件**（相对路径 vs 带前导斜杠的排除片段，见第八节）。
3. **判据依赖的名单/基线取错了**，永远取到空集。

所以本文件用**动态**办法补上：给每道门注入一次最小违规，跑门，断言它红。注入前是绿的、
注入后红了，才算真门（若注入前就红 ⇒ 工作区不干净，会提示「验证不充分」）。

它抓到过的真问题：`ignore_gate` 的正则原先锚了行首（`^\s*let…`），
`pub fn f() { let _ = g(); }` 这种**单行函数体里的丢弃整个漏过去**——注入探针才发现。

归 `full` 档：每道门跑两遍（注入前 / 后）≈ 24 秒，不进 pre-commit；CI 的 `gates.yml`
显式跑它。临时探针落在 `scripts/gates/_probe/`（跑完自删，也在 `.gitignore` 里）。
