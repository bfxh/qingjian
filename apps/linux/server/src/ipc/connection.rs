...
pub(super) fn serve_connection(
    ...
    // SAFETY: `credentials` 是本栈上的 `libc::ucred`，`size` 先初始化为 `size_of::<ucred>()`
    // ——内核按 Linux 的 SO_PEERCRED 契约只写一个 `struct ucred` 并回填实际长度；返回值先判
    // `== 0` 再读字段，失败时结构体保持全 0（不会读到未初始化内存）；指针取自本地变量、
    // 调用期间不外逃；fd 由 `stream` 持有，调用期间有效。
    let trusted = unsafe {
        libc::getsockopt(
            stream.as_raw_fd(),
            libc::SOL_SOCKET,
