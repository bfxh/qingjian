...
cargo clippy --all-targets -- -D warnings   # dbg!/todo!/unimplemented!/panic!/unreachable! 已 deny
cargo fmt --all
...
- TODO/FIXME/HACK/XXX 残留：`python3 scripts/detect_todos.py`（pre-commit 与 quality.yml 都跑）。
- 依赖重复版本：`cargo deny check bans` 要绿——新重复先进 deny.toml 的 skip 登记再谈合入。
- Miri（周度，audit.yml）：`cargo +nightly miri test -p qingjian-core --lib`。
