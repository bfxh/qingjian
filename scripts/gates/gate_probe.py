...
# 需要复制一份现有文件才能触发的（雷同门）
COPIES = [("dupe-gate", "dupe_gate.py", "crates/qingjian-core/src/lib.rs")]

# 需要往**已有**文件里注入的（写 .agents/claims 下的声明）
EXTRA = [("agent-gate", "agent_gate.py", ROOT / ".agents" / "claims" / "probe-invalid.json",
          '{"agent":"someone-else","task":"探针","scope":["zzz-nonexistent/**"],'
          '"expires":"2026-09-27"}\n')]
