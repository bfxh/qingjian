...
def main() -> int:
    ...
    for agent, names in by_agent.items():
        if len(names) > 1:
            fails.append(f"A2 智能体 {agent} 有 {len(names)} 份声明 {names}——一个智能体只许一份")
    ...
    # A4/A5 身份与越界
    who = a.agent or os.environ.get("QJ_AGENT_ID") or _git("config", "--get", "qingjian.agentId").stdout.strip() or None
    if who is None:
        ...
        if a.changed_from == "worktree":
            changed = set(_git("diff", "--name-only", "HEAD").stdout.split())
        elif a.changed_from == "none":
            changed = set()
        else:
            changed = set(_git("diff", "--name-only", a.changed_from).stdout.split())
        ...
        if changed and my_scope:
            outside = [f for f in sorted(changed)
                       if not any(match(f, p) for p in my_scope | {".agents/**", "scripts/gates/**",
                                                                   ".github/workflows/gates.yml",
                                                                   "docs/review/god-*.md",
                                                                   "docs/review/god-*.json",
                                                                   "docs/review/dupe-baseline.json"})]
            for f in outside[:10]:
